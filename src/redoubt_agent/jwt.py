...
def fetch_jwt(claims=None, expiresAt=None) -> str:
    if claims is None:
        claims = {}
    if(os.environ['NODE_ENV'] != 'production'):
        return MOCK_JWT

    host_name = 'redoubt-jwt-provider'
    port = 8515
    path = '/create'
...
def verify_jwt(token: str, tonUrl: str = 'https://ton-rpc.com') -> bool: # FIXME
    ...
    w3Client = Web3(Web3.HTTPProvider(tonUrl))
